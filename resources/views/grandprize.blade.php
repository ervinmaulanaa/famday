...
<head>
    ...
    <style>
        ...
        #scene {
            ...
            margin-top: -300px;
            margin-left: -600px;
            background: url(/undian/11.jpg) no-repeat center center fixed;
                    ...
                    <div class="reel">
                        <div id="planeMachine">
                            @foreach ($dtnya as $k => $v)
                                <p id="{{ $k }}" pid="{{ $v->peserta_id }}" phone="{{ $v->person_phone }}"
                                    no="{{ $v->peserta_no }}" nama="{{ strtoupper($v->peserta_nama) }}">
                                    {{ $v->peserta_no }}</p>
                            @endforeach
                        </div>
                    </div>
